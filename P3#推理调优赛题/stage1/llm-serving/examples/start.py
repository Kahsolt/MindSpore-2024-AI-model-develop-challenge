...
if __name__ == "__main__":
    ...
    p_agent = subprocess.Popen([f'python examples/start_agent.py --task {args.task} --config {args.config}> agent.log 2>&1 &'], shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1)
    agents_finished = False
    while not agents_finished:
        # check if agent pid is alive
        if not psutil.pid_exists(p_agent.pid) or (p_agent.pid not in psutil.pids()):
    ...
    p_server = subprocess.Popen([f'python examples/server_app_post.py --config {args.config}> server.log 2>&1 &'], shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1)
    server_finished = False
    while not server_finished:
        # check if server pid is alive
        if not psutil.pid_exists(p_server.pid) or (p_server.pid not in psutil.pids()):
